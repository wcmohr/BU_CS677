...
def classifier_name(clf):
    return clf.__str__().split('(')[0]

def compile_results(results, model, predictions, true, model_name=None, trading=0, year=2021, df=None):
    """take results thus far and add statistics for the current model.  
    Columns: ['TP','FP','TN','FN','accuracy','TPR','TNR'].  If trading != 0, also include column with returns on $100 for year (default 2021) and set df as the weekly return data for the stock."""
    cm = confusion_matrix(true,predictions)
    if not model_name:
        model_name = classifier_name(model)
    tp = np.round(cm[1,1],0)
    fp = np.round(cm[0,1],0)
    tn = np.round(cm[0,0],0)
    fn = np.round(cm[1,0],0)
    accuracy = (tp + tn) / (tp + tn + fp + fn)
    tpr = tp / (tp + fn) # sensitivity
    tnr = tn / (tn + fp) # recall
    if trading:
        results.loc[model_name] = [tp,fp,tn,fn,np.round(accuracy,2),np.round(tpr,2),np.round(tnr,2),trade_labels(df,year,np.where(predictions=='Green',1,0))]
        results = results.astype({'TP': 'int', 'FP':'int', 'TN':'int', 'FN':'int'})
    else:
        results.loc[model_name] = [tp,fp,tn,fn,np.round(accuracy,2),np.round(tpr,2),np.round(tnr,2)]
        results = results.astype({'TP': 'int', 'FP':'int', 'TN':'int', 'FN':'int'})
    return results
